...
def get_leaderboard():
    contract, caller, nonce, private_key = initialize_smart_contract(LEADERBOARD_CONTRACT_ADD, LEADERBOARD_ABI)
    Chain_id = web3.eth.chain_id
    list_of_uuids = contract.functions.getUuids().call()

    list_of_scores = []
    for uuid in list_of_uuids:
        score = contract.functions.getScore(uuid).call()
        list_of_scores.append({"uuid": uuid, "score": score})
